...
int main(int argc, char** argv) {
	...
	for (i = 0; i < x; i++) {
		for (j = 0; j < x; j++) {
			printf("\t\tc[threadIdx.x][%d][%d] = max(a[threadIdx.x][%d],c[threadIdx.x][%d][%d]+b_%d);\n",i,j,x-i-1,i,j,j);
		}
	}
	printf("\n\n");
 	for (i = 0; i < x; i++) {
        printf("\t\tb_%d = max(b_%d,(a[threadIdx.x][%d]+c[threadIdx.x][%d][%d]));\n",i,i,i,1,i);
    }
	printf("\n\n");
	for (i = 0; i < x; i++) {
        printf("\t\ta[threadIdx.x][%d] = max(a[threadIdx.x][%d],(b_%d+c[threadIdx.x][%d][%d]));\n",i,i,i,0,i);
    }
