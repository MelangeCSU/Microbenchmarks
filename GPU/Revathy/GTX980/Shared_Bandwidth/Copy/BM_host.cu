...
int main(int argc, char ** argv)
{
    ...
    */
    ...
    double nOps = vectorLen*iters*0.0;
    double nOpsPerSec = nOps/(time/1000.0); //1e3*nOps/time;
    double nGOpsPerSec = nOpsPerSec*1e-9;
    
    // Compute transfer rates.
    double nBytes = (double)(2.0*vectorLen*iters)*(double)sizeof(int); // 2N integer inputs + N integer output
    ...
    printf("%d\t%d\t%d\t%d\t%f\t%f\t%f\n",x, iters, B, T, nGOpsPerSec, nGBytesPerSecPerSM, time/1000.0);
