...
int main(int argc, char ** argv)
{
    ...
    if(argc != 6){
     ...
     printf("Repeats is the number of times the main loop is repeated in the device kernel.\n");
    ...
    long size = vectorLen*sizeof(int);
    /*
    h_A = (int*)malloc(size);
    ...
    */
    ...
    double nOps = vectorLen*iters*2.0;
    double nOpsPerSec = nOps/(time/1000.0); //1e3*nOps/time;
    double nGOpsPerSec = nOpsPerSec*1e-9;
    
    // Compute transfer rates.
    double nBytes = (double)(4.0*vectorLen*iters)*(double)sizeof(int); // 2N integer inputs + N integer output
    ...
    printf("%d\t%d\t%d\t%d\t%f\t%f\t%f\t%d\n",x, iters, B, T, nGOpsPerSec, nGBytesPerSecPerSM, time/1000.0, u);
