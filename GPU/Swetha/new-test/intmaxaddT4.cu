...
int main(int argc, char** argv)
{
	...
	error = cudaMalloc((void**)&d_C, size_C*sizeof(int));
   	...
   	IntmaxaddT4<<<dimGrid, dimBlock>>>(d_C, k , sl,ValuesPerThread);
	...
	double nops = (double)size_C*(double)2*(double)k+ (double)2*(double)k*(double)gridWidth*(double)blockWidth;
