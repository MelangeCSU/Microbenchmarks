...
int main(int argc, char** argv) {
	...
	printf("__global__ void IntmaxaddF1(const int* A, const int* B, int* C, int x, int k)\n{\n");
	...
	for (i = 0; i < x; i++) {
		for (j = 0; j < x; j++) {
			printf("\t\tc_%d_%d = max(a_%d,c_%d_%d+b_%d);\n",i,j,i,i,j,j);
		}
	}
	printf("\n\n");
	for (i = 0; i < x; i++) {
	        printf("\t\ta_%d = max(a_%d,b_%d+c_%d_%d);\n",i,i,x-i-1,i,x-i-1);
	
    	}
    	printf("\n\n");

    	for (i = 0; i < x; i++) {
	        printf("\t\tb_%d =  max(b_%d,a_%d+c_%d_%d);\n",i,i,x-i-1,i,x-i-1);
		//printf("\t\tb_%d =  max(a_%d,b_%d+c_%d_%d);\n",i,i,x-i-1,i,x-i-1);
	}
    	printf("\n\n");
