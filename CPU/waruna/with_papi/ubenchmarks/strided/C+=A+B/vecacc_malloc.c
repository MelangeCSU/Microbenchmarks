...
*/	
	...
	double bw = (double)((double)ARRAY_SIZE*(double)REPS)*(double)4*(double)4*(double)tds/vector_time/((double)1e9);
	double gops01 = (double)((double)ARRAY_SIZE*(double)REPS*(double)tds*(double)2)/vector_time_01/((double)1e9);
	double bw1 = (double)((double)ARRAY_SIZE*(double)REPS)*(double)4*(double)4*(double)tds/vector_time_01/((double)1e9);
  ...
  __m128i a0,a1,a2,a3,b0,b1,b2,b3;
	...
	for (k = 0; k < REPS; k++) 
	{
		for (itr = 0; itr<ARRAY_SIZE; itr+=16)
		{
			...
			c0 = _mm_load_si128((__m128i*)&cc[itr]);
			c1 = _mm_load_si128((__m128i*)&cc[itr+4]);	
			c2 = _mm_load_si128((__m128i*)&cc[itr+8]);	
			c3 = _mm_load_si128((__m128i*)&cc[itr+12]);	

			c0 = _mm_max_epi32(_mm_add_epi32(a0, b0),c0);
			c1 = _mm_max_epi32(_mm_add_epi32(a1, b1),c1);
			c2 = _mm_max_epi32(_mm_add_epi32(a2, b2),c2);
			c3 = _mm_max_epi32(_mm_add_epi32(a3, b3),c3);
