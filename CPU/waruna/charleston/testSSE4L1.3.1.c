...
int oneThread(int threadId, int w, int nn, long iReps)
{
	int *cc __attribute__((aligned(16)));
	int i;
	long k;
	int itr;
	int w2 = 2*w;

	cc = (int *)_mm_malloc(sizeof(int)*nn, 16);
...
*/
	...
	for (k = 0; k < iReps; k++) 
	{
		for (itr = nn; itr>w2; itr-=w2)
		{
			...
			c0 = _mm_max_epi32(_mm_add_epi32(b0,a), c0);
			c1 = _mm_max_epi32(_mm_add_epi32(b1,a), c1);
			c2 = _mm_max_epi32(_mm_add_epi32(b2,a), c2);
			c3 = _mm_max_epi32(_mm_add_epi32(b3,a), c3);
...
*/
			...
			b0 = _mm_max_epi32(_mm_add_epi32(c0,a), b0);
			b1 = _mm_max_epi32(_mm_add_epi32(c1,a), b1);
			b2 = _mm_max_epi32(_mm_add_epi32(c2,a), b2);
			b3 = _mm_max_epi32(_mm_add_epi32(c3,a), b3);
